...
def main():
    ...
    for key in rank.keys():
        rank[key] = sorted(rank[key].items(), key=lambda d: d[1], reverse=True)
    ...
    ax.legend(handles=legend_elements, loc='center left',
              markerscale=20, prop=ChineseFont1, fontsize='x-small',
              bbox_to_anchor=(1, 0.5))

    #Create the map
    m = Basemap(resolution='f', projection='merc', llcrnrlat=y1,
                urcrnrlat=y2, llcrnrlon=x1, urcrnrlon=x2, lat_ts=0)
    ...
    plt.title('Freq_by_region', fontsize=10, weight='bold')
    ...
    for key in rank.keys():
        ...
        draw_pie(rank[key], top, ax, X, Y, size=300)

    #output charts
    plt.savefig('freq_by_region_pies.png', dpi=2000)
